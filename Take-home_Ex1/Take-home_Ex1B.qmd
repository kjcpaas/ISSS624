...
# Creating spatial data for each peak period
...
kable(head(peak_wkdy_am))
```

### Weekday PM (5 - 8 PM)

```{r}
peak_wkdy_pm <- trips_cube_wkdy %>%
  filter(
    HOUR_OF_DAY >= 17 &
      HOUR_OF_DAY < 20
  ) %>%
  group_by(HEX_ID) %>%
  summarise(TRIPS = sum(TRIPS))
kable(head(peak_wkdy_pm))
```

### Weekend AM (11 AM - 2 PM)

```{r}
peak_wknd_am <- trips_cube_wknd %>%
  filter(
    HOUR_OF_DAY >= 11 &
      HOUR_OF_DAY < 14
  ) %>%
  group_by(HEX_ID) %>%
  summarise(TRIPS = sum(TRIPS))
kable(head(peak_wknd_am))
```

### Weekend PM (4 - 7 PM) {#weekend-pm-4---7-pm}

```{r}
peak_wknd_pm <- trips_cube_wknd %>%
  filter(
    HOUR_OF_DAY >= 16 &
      HOUR_OF_DAY < 19
  ) %>%
  group_by(HEX_ID) %>%
  summarise(TRIPS = sum(TRIPS))
kable(head(peak_wknd_pm))
```
:::

## Adding geometry data

To plot the maps, we need to do add geometry to our spatial data. This can be done by doing a `left_join` of our geometry to the aspatial data generated in [Creating spatial data for each peak period].

We also need to covert this to a `sf` data frame to be processed properly as spatial data.

::: panel-tabset
### Weekday AM (6 - 9 AM)

```{r}
peak_wkdy_am_sf <- left_join(peak_wkdy_am, honeycomb) %>% st_sf()
kable(head(peak_wkdy_am_sf))
```

### Weekday PM (5 - 8 PM)

```{r}
peak_wkdy_pm_sf <- left_join(peak_wkdy_pm, honeycomb) %>% st_sf()
kable(head(peak_wkdy_pm_sf))
```

### Weekend AM (11 AM - 2 PM)

```{r}
peak_wknd_am_sf <- left_join(peak_wknd_am, honeycomb) %>% st_sf()
kable(head(peak_wknd_am_sf))
```

### Weekend PM (4 - 7 PM)

```{r}
peak_wknd_pm_sf <- left_join(peak_wknd_pm, honeycomb) %>% st_sf()
kable(head(peak_wknd_pm_sf))
```
:::
...
We do not need the `peak_wk*_*` data anymore as contents are already in the sf data. We will remove them from the R environment so we can see the relevant data more easily.
...
quantile(
  c(
    peak_wkdy_am_sf$TRIPS,
    peak_wkdy_pm_sf$TRIPS,
    peak_wknd_am_sf$TRIPS,
    peak_wknd_pm_sf$TRIPS
  ),
