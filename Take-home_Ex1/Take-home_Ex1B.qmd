...
To make it easier to generate maps, we will use the following utility function so we don't need to repeat the same thing all over again.
...
generate_common_legend <- function(source, breaks, position = c("right", "bottom")) {
  tm_shape(source) +
    tm_fill(
      "WEEKDAY_AM_TRIPS", # we can use any column here
      palette = "YlOrRd",
      title = "# of trips",
      breaks = breaks
    ) +
...
However, we need to do a deeper spatial analysis like LISA or EHSA to have more detailed insights on spatial patterns as we can only get an overview of patterns from visual inspection.
:::
