...
For demonstrating the steps, we will use the **August 2023** data set. The same steps will be applied to the other data sets later on.
:::
...
We do not need `odbus202308` anymore as we will be working with the more lightweight `trips202308` from this point on.
...
To achieve our goal of having the hourly \# of bus trips per location, we need to add `HEX_ID` to `trips202308` data. This is so we can answer, *how many bus trip originate from a certain hexagon?*

To do this, we will do an `inner_join()` to join the `trips202308` data with `bs_hex`.

::: {.callout-tip collapse="true"}
### Why \`inner_join()\` instead of \`left_join()\`?

We will use `inner_join` as there are `BUS_STOP_N` values in `trips202308` data that are not in `bs_hex`.

```{r}
trips202308$BUS_STOP_N[!(trips202308$BUS_STOP_N %in% bs_hex$BUS_STOP_N)] %>%
  unique() %>% length()
```

There are **57** bus stops in `trips202308` that are not in `bs_hex`. **5** of this can be attributed the bus stops we removed in [About those points outside Singapore]. Others may be due to the *BusStops* data set not having complete information.
...
Therefore, we will use `inner_join` to keep only the observations in `trips202308` with the matching bus stops in `bs_hex`.
:::

```{r}
trips202308 <- inner_join(trips202308, bs_hex)
kable(head(trips202308))
```
...
`trips202308` was processed from *Passenger Volume By Origin Destination Bus Stops*, which has almost **6 million** observations.

We now have a more lightweight dataset with almost **60,000** observations, which is about **100x smaller**.

Let's save this data as an `rds` file so we don't need to reprocess everytime later on.

```{r}
write_rds(trips202308, "data/rds/trips202308.rds")
```
:::

## Transforming the September and October data

Now that `odbus202308` is in the form we want, we will apply the same methods for the September and October data.

::: {.callout-important appearance="simple"}
This will do a quick run-through of the code. To understand the thought process, please read from [Extracting Hourly \# of Bus Trips Originating from Hexagons](#extracting-hourly-of-bus-trips-originating-from-hexagons).
:::
...
```{r}
trips202308 <- read_rds("data/rds/trips202308.rds")
...
## Is our *trips202308* **data time series cube-friendly? The answer is NO.**

```{r}
spacetime(trips202308, honeycomb,
          .loc_col = "HEX_ID",
...
```{r}
trips_wkdy202308 <- trips202308 %>%
  filter(DAY_TYPE == "WEEKDAY") %>%
  ungroup() %>%
  select(, -DAY_TYPE)
kable(trips_wkdy202308[20:29,])
```

### Weekend/Holidays

```{r}
trips_wknd202308 <- trips202308 %>%
  filter(DAY_TYPE == "WEEKENDS/HOLIDAY") %>%
  ungroup() %>%
  select(, -DAY_TYPE)
kable(trips_wknd202308[20:29,])
```
:::
...
Let's check the total rows in `trips_wkdy202308` and `trips_wknd202308` add up to the number of rows in `trips202308`.

```{r}
nrow(trips_wkdy202308) + nrow(trips_wknd202308) == nrow(trips202308)
```
...
We do not need `trips202308` anymore as we will be using `trips_wkdy202308` and `trips_wknd202308` from this point.
...
Now that we have separate data frames for weekday and weekend/holiday, we need to make sure that our data frame as all the combination in `combos`. We can do that by joining `trips_wkxx202308` with `combos`.

::: panel-tabset
### Weekday

```{r}
trips_cube_wkdy202308 <- left_join(combos, trips_wkdy202308)
kable(head(trips_cube_wkdy202308, n = 24))
```

Check if the output has the same rows as `combos`.

```{r}
nrow(trips_cube_wkdy202308) == nrow(combos)
```

### Weekend/Holiday

```{r}
trips_cube_wknd202308 <- left_join(combos, trips_wknd202308)
kable(head(trips_cube_wknd202308, n = 24))
```

Check if the output has the same rows as `combos`.

```{r}
nrow(trips_cube_wknd202308) == nrow(combos)
```
:::
...
We do not need `trips_wkxx202308` anymore as we will be using `trips_cube_wkxx202308` from this point on.
...
```{r}
trips_cube_wkdy202308$TRIPS[is.na(trips_cube_wkdy202308$TRIPS)] <- 0
kable(head(trips_cube_wkdy202308, n = 24))
```
...
```{r}
trips_cube_wknd202308$TRIPS[is.na(trips_cube_wknd202308$TRIPS)] <- 0
kable(head(trips_cube_wknd202308, n = 24))
```
:::
...
```{r}
spacetime(trips_cube_wkdy202308, honeycomb,
          .loc_col = "HEX_ID",
          .time_col = "HOUR_OF_DAY") %>%
  is_spacetime_cube()
```
...
```{r}
spacetime(trips_cube_wknd202308, honeycomb,
          .loc_col = "HEX_ID",
          .time_col = "HOUR_OF_DAY") %>%
  is_spacetime_cube()
```
...
`trips_cube_wkxx202308` data is part the [Goal data sets] we need. Hence, we will save them.

```{r}
write_rds(trips_cube_wkdy202308, "data/rds/trips_cube_wkdy202308.rds")
write_rds(trips_cube_wknd202308, "data/rds/trips_cube_wknd202308.rds")
```
:::

## Processing the September and October data
