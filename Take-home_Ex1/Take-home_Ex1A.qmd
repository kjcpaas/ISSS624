...
::: callout-important
This document covers only the **Data Wrangling** part of the study.
:::
...
This can be done using `st_intersection()`. We will also drop the geometry here using `st_drop_geometry()` and make the data spatial as the resulting mapping will be use as the **connector** to join the aspatial bus commuter data with the hexagons.
...
kable(head(hourly_from_bs202310, n = 30))
```
...
bus_peaks_hc202310$WEEKDAY_PM_TRIPS[is.na(bus_peaks_hc202310$WEEKDAY_PM_TRIPS)] <- 0
kable(bus_peaks_hc202310[121:130,])
```
...
bus_peaks_hc202310$WEEKEND_AM_TRIPS[is.na(bus_peaks_hc202310$WEEKEND_AM_TRIPS)] <- 0
kable(bus_peaks_hc202310[121:130,])
```
...
bus_peaks_hc202310$WEEKEND_PM_TRIPS[is.na(bus_peaks_hc202310$WEEKEND_PM_TRIPS)] <- 0
kable(bus_peaks_hc202310[121:130,])
```
...
bus_peaks_hc202308$WEEKDAY_AM_TRIPS[is.na(bus_peaks_hc202308$WEEKDAY_AM_TRIPS)] <- 0
rm(weekday_am)
...
bus_peaks_hc202308$WEEKDAY_PM_TRIPS[is.na(bus_peaks_hc202308$WEEKDAY_PM_TRIPS)] <- 0
rm(weekday_pm)
...
bus_peaks_hc202308$WEEKEND_AM_TRIPS[is.na(bus_peaks_hc202308$WEEKEND_AM_TRIPS)] <- 0
rm(weekend_am)
...
bus_peaks_hc202308$WEEKEND_PM_TRIPS[is.na(bus_peaks_hc202308$WEEKEND_PM_TRIPS)] <- 0
rm(weekend_pm)
...
bus_peaks_hc202309$WEEKDAY_AM_TRIPS[is.na(bus_peaks_hc202309$WEEKDAY_AM_TRIPS)] <- 0
rm(weekday_am)
...
bus_peaks_hc202309$WEEKDAY_PM_TRIPS[is.na(bus_peaks_hc202309$WEEKDAY_PM_TRIPS)] <- 0
rm(weekday_pm)
...
bus_peaks_hc202309$WEEKEND_AM_TRIPS[is.na(bus_peaks_hc202309$WEEKEND_AM_TRIPS)] <- 0
rm(weekend_am)
...
bus_peaks_hc202309$WEEKEND_PM_TRIPS[is.na(bus_peaks_hc202309$WEEKEND_PM_TRIPS)] <- 0
rm(weekend_pm)
...
├── sg_honeycomb.rds
└── sg_honeycomb_with_num_busstops.rds
```

We will be using these data files in the next part, [Visualization and Analysis](/Take-home_Ex1/Take-home_Ex1B.qmd).
