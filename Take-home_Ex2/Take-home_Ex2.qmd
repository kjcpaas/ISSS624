...
We will also prepare the `data/rds/` directory to save important or expesive data, such as those we load from our [Load point].

After the setup we are left with the directory structure below.
...
│   ├── MPSZ-2019.shp
│   ├── MPSZ-2019.shx
│   ├── Train_Station_Exit_Layer.cpg
│   ├── Train_Station_Exit_Layer.dbf
│   ├── Train_Station_Exit_Layer.lyr
│   ├── Train_Station_Exit_Layer.prj
│   ├── Train_Station_Exit_Layer.sbn
│   ├── Train_Station_Exit_Layer.sbx
│   ├── Train_Station_Exit_Layer.shp
│   ├── Train_Station_Exit_Layer.shp.xml
│   ├── Train_Station_Exit_Layer.shx
│   ├── Retails.cpg
│   ├── Retails.dbf
│   ├── Retails.prj
│   ├── Retails.qix
...
-   set **seed** for the *random number generator* so our results are deterministic (for reproduceability)
:::
...
#| label: setup
pacman::p_load(
  sf, sp, tmap, tidyverse, knitr,
  stplanr, reshape2, performance,
  ggpubr, corrplot, MASS)
tmap_mode("plot")
tmap_style("natural")
set.seed(1234)
```
...
## Plotting *TRIPS* vs. *DISTANCE* {#sec-plotting-trips-vs-distance}
...
::: {.callout-tips collapse="true" appearance="minimal"}
#### Insights
...
### Performance via RMSE

[Root Mean Squared Error (RMSE)](https://www.statisticshowto.com/probability-and-statistics/regression-analysis/rmse-root-mean-square-error/) is a measure of how spread out the values are from the best-fit line.
...
For example, from [plotting TRIPS vs DISTANCE](#sec-plotting-trips-vs-distance), we observed outliers in the plot, i.e. zone pairs that have long distances but on the top 5% highest number of trips. The same can be observed in [Plotting the fit]. Clearly, there are other forces that result in bus trips, other than distance, that are not uncovered by the models we have.
...
From [Spatial Interaction Modelling], we found that the **doubly constrained model** is the best out of all the models we created so far. In this part, we will try to improve the models, with the doubly constrained metrics as baseline:

**R-squared**: 0.6103781

**RMSE:** 236.1052

As the doubly constrained model does not use explanatory variables other than distance, we will only focus our improvements on the three other models that consider the attractive and propulsive forces: **origin constrained**, **destination constrained**.

::: callout-caution
As I am no statistics expert and do not have research background, the techniques here are based on common sense and are not based on best practices in the industry.

I just want to explore if doing these steps can improve the initial models, for my own curiosity.
:::
...
We are using [Poisson regression](https://stats.oarc.ucla.edu/r/dae/poisson-regression/) method, which is approriate for when the mean and variance of the dependent variable are not extremely different.
...
```{r}
SIM_data <- read_rds("data/rds/SIM_data_no_intra202310.rds")
uncSIM <- read_rds("data/rds/uncSIM202310.rds")
dbcSIM <- read_rds("data/rds/dbcSIM202310.rds")
```
:::
...
We will clear that were used in this investigation data we do not need anymore.

```{r}
rm(model_list_corr)
rm(SIM_data_corr_lt_06)
rm(SIM_data_corr_lt_08)
rm(uncSIM_corr_lt_06)
rm(uncSIM_corr_lt_08)
```
:::
...
Next, we will use the **negative binomial** regression method. We will use the `glm.nb()` function from [MASS](https://cran.r-project.org/web/packages/MASS/index.html) package.
...
```{r}
#| eval: false
uncSIM_nb <- glm.nb(formula = TRIPS ~ 
                      log(ORIG_HDB_RESIDENT_COUNT) +
                      log(ORIG_BUS_ALIGHT_COUNT) +
                      log(ORIG_TRAIN_ALIGHT_COUNT) +
                      log(DEST_BUS_STOP_COUNT) +
                    ...
                    data = SIM_data,
                    na.action = na.exclude)
write_rds(uncSIM_nb, "data/rds/uncSIM_nb202310.rds")
```

Coefficients for our explanatory variables:

```{r}
uncSIM_nb <- read_rds("data/rds/uncSIM_nb202310.rds")
kable(tail(uncSIM_nb$coefficients, n = 11))
```
...
We can notice that the coefficients are different with `ORIG_TRAIN_ALIGHT_COUNT` being **positive** now, implying that the more passengers alight from train, the more bus rides there are.

This is contrary to our original model.

`DEST_BUS_STOP_COUNT` also switched coefficient signs, implying that presence of bus stops at the destination deter people from going there.
:::
...
```{r}
#| eval: false
orcSIM_nb <- glm.nb(formula = TRIPS ~ 
                      ORIG_HEX_ID +
                      log(DEST_BUS_STOP_COUNT) +
                      log(DEST_ENTERTN_COUNT) +
                      log(DEST_F_AND_B_COUNT) +
                    ...
                    data = SIM_data,
                    na.action = na.exclude)
write_rds(orcSIM_nb, "data/rds/orcSIM_nb202310.rds")
```

Coefficients for our explanatory variables:

```{r}
orcSIM_nb <- read_rds("data/rds/orcSIM_nb202310.rds")
kable(tail(orcSIM_nb$coefficients, n=8))
```
...
We can notice that the sign for `DEST_LEISURE_COUNT` coefficient switched from one model to the next, implying the attractiveness of leisure destinations in this model.
:::
...
```{r}
#| eval: false
decSIM_nb <- glm.nb(formula = TRIPS ~ 
                      DEST_HEX_ID +
                      log(ORIG_HDB_RESIDENT_COUNT) +
                      log(ORIG_BUS_ALIGHT_COUNT) +
                      log(ORIG_TRAIN_ALIGHT_COUNT) +
                 ...
                 data = SIM_data,
                 na.action = na.exclude)
write_rds(decSIM_nb, "data/rds/decSIM_nb202310.rds")
```
...
```{r}
#| eval: false
decSIM_nb <- read_rds("data/rds/decSIM_nb202310.rds")
kable(tail(decSIM_nb$coefficients, n=4))
```
...
Not able to generate due to time constraints. Model was taking almost an hour to generate.
:::
...
dbcSIM_nb <- glm.nb(formula = TRIPS ~
                    ...
                    data = SIM_data,
                    na.action = na.exclude)
write_rds(dbcSIM_nb, "data/rds/dbcSIM_nb202310.rds")
```
...
```{r}
#| eval: false
dbcSIM_nb <- read_rds("data/rds/dbcSIM_nb202310.rds")
kable(tail(dbcSIM_nb$coefficients, n=1))
```
...
We will also load the `calc_r_squared()` function for model comparison.
...
Now, let's compare the performance of the negative binomial models that we were able to generate from the original ones.

```{r}
model_list <- list(
  Unconstrained = uncSIM,
  Unconstrained_NB = uncSIM_nb,
  Origin_Constrained = orcSIM,
  Origin_Constrained_NB = orcSIM_nb,
  Destination_Constrained = decSIM,
  Doubly_Constrained = dbcSIM)
```
...
Using the **negative binomial regression** resulted in a **higher RMSE**, which means that it is **worse** than **Poisson regression**.

Despite our data being overdispersed, using a different regression **did not improve** the model fit. Hence, the best model so far is still the **doubly constrained model using Poisson regression**.

Other factors may not be covered by simple spatial interaction models, and may be explained better by **Spatial Economic Interaction Models**.
:::
