...
However, the result of the modelling thus far does that mean that this is the best reflection of the real-world scenario. It just means that the **alternative models do a worse job of modelling**.

For example, from [plotting TRIPS vs DISTANCE](#sec-plotting-trips-vs-distance), we observed outliers in the plot, i.e. zone pairs that have long distances but on the top 5% highest number of trips. There are other forces that result in those trips other than the distance that are not uncovered by the models we have.
