...
::: {.callout-note appearance="simple"}
We will **not distribute** these taps to the number of exits as we cannot be accurate on the volume of people taking specific exits. We can consider the people tapping out from a station **as potential passengers** of bus stop from the nearby exits.
:::
...
Let's save `tap_out_hex` as it contains the data for propulsiveness variable.

```{r}
write_rds(tap_out_hex, "data/rds/tap_out_hex202310.rds")
```
:::
...
tap_out_hex <- read_rds("data/rds/tap_out_hex202310.rds")
```
:::
...
```{r}
propulsiveness <- propulsiveness %>%
  left_join(st_drop_geometry(tap_out_hex)) %>%
  rename(TRAIN_ALIGHT_COUNT = TAPS)
```
:::
...
Now that we have all the components, we will now generate the aspatial `data` that contains the following columns:
...
`dist_tbl` covers all the pairs of hexagons from `honeycomb`. Hence, its number of rows is equal to the **square of number of hexagons** in `honeycomb`.

```{r}
nrow(dist_tbl) == nrow(honeycomb)^2
```

With that, we will use it as the base for `model_data`.
...
We will get the number of trips from `od_hex` and will join it with `model_data`.
...
Propulsive forces come from the origin zone so we will join `propulsiveness` with `model_data` on `ORIG_HEX_ID`. We will also add the `ORIG_` prefix to the columns added in by `propulsiveness`.
...
Attractive forces come from the destination zone so we will join `attractiveness` with `model_data` on `DEST_HEX_ID`. We will also add the `DEST_` prefix to the columns added in by `attractiveness`.
...
We can now remove the data that we use to derive `model_data` from the environment.
...
Let's take a look at our `model_data`.

```{r}
summary(model_data)
```

We will make finishing touches on `model_data`, so that they are compatible with modelling. We need to remove the 0's as we will apply *log* function to them, which will result to *undefined*. We will set them to **0.99**

```{r}
model_data[model_data == 0] <- 0.99
```

Also, some values of `TRIPS` are `NA`. We will also set them to `0`. We are not applying *log* to `TRIPS` in the model as it is the dependent variable so they are fine to be `0`.
...
Let's save `model_data` as it contains the data we need for our models.
...
We won't need the `model_data` for a while so we can remove it from the environment.
...
model_data <- read_rds("data/rds/model_data202310.rds")
propulsiveness <- read_rds("data/rds/propulsiveness202310.rds")
attractiveness <- read_rds("data/rds/attractiveness.rds")
```
:::
...
#| code-fold: true
#| code-summary: "**Show the code**"
ggplot(model_data %>% filter(TRIPS > 0),
...
As we can see in this plot, the distance decay can still be seen in the plot, as the *maximum number of trips* decrease as the distance increases.
...
plot_propulsive <- function(var_name, title_comp) {
  ...
  tm_shape(propulsiveness %>% filter(across(var_name, ~. > 0))) +
  tm_polygons(var_name, palette = "Greens", style = "quantile") +
