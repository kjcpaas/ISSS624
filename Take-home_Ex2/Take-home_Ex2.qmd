...
::: {.callout-important appearance="simple"}
I originally wanted to perform a negative binomial using `glm.nb()` from [MASS](https://www.rdocumentation.org/packages/MASS/versions/7.3-58.3) but after importing the package, it broke the functionality of `select()`.

Perhaps the package redefines `select()`. I need more time to investigate the root cause of this.

Unfortunately, I am not able to show the code chunks anymore.

However, the models resulted in a higher RMSE (300+ compare to 200+ in the original models) so having a model more suited to an over-dispersed data does not necessarily produce a better model.
:::

# Conclusion

The result of the modelling thus far does that mean that this is the best reflection of the real-world scenario. This is even after trying negative binomial regression that is better suited for the an over-dispersed dataset like ours.

To explain the outliers we observed [plotting TRIPS vs DISTANCE](#sec-plotting-trips-vs-distance) and [Plotting the fit], we may need to perform a different analysis like **Spatial Economic Interaction Models**, or add weights to the different destination i.e., Kranji and Woodlands Checkpoint bus stops are very attractive because of access to Malaysia.
